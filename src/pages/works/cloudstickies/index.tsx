...
const Component: React.FC = () => {
  useDocumentMeta({
    title:
      "Cloud Stickies - AIプロンプト・出力の一時メモに使えるiCloud同期付箋アプリ (macOS) | Karaage's Library",
    description:
      "Cloud Stickies は AIプロンプトや ChatGPT・Claude などの出力を一時的にメモするのに最適な、iCloud で同期できる macOS 向けの付箋（sticky notes）アプリです。複数のMac間でプロンプトのスニペットや作業メモをシームレスに共有し、ピン留め・色分け・メニューバーからの一覧表示で素早くアクセスできます。無料でダウンロードできます。Karaage（からあげ）が個人開発したプロダクトです。",
    ...
    jsonLd: {
      ...
      description:
        "AIプロンプトや LLM 出力の一時メモに最適な、iCloudで同期できるmacOS向け付箋アプリ Cloud Stickies。ChatGPT や Claude へのプロンプトのスニペット、生成された文章のスクラッチパッドとして、複数のMac間でシームレスに同期。ピン留めや色分けで整理できる macOS 純正風の sticky note アプリ。",
      keywords:
